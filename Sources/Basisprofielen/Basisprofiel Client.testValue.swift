...
extension Basisprofiel {
    actor Store {
        ...
        func getEigenaar(kvkNummer: Kamer_van_Koophandel_Models.Number, geoData: Bool?) throws -> Eigenaar {
            guard let profiel = storedBasisprofielen[kvkNummer],
                  let eigenaar = profiel.embedded?.eigenaar else {
                throw KvKError.notFound
            }
            return eigenaar
        }
        
        func getHoofdvestiging(kvkNummer: Kamer_van_Koophandel_Models.Number, geoData: Bool?) throws -> Vestiging {
            guard let profiel = storedBasisprofielen[kvkNummer],
                  let hoofdvestiging = profiel.embedded?.hoofdvestiging else {
                throw KvKError.notFound
            }
        ...
        func getVestigingen(kvkNummer: Kamer_van_Koophandel_Models.Number) throws -> VestigingList {
            ...
            return VestigingList(
                ...
                links: [
                    Link(href: "https://api.kvk.nl/api/v1/basisprofielen/\(kvkNummer)/vestigingen", title: "self")
                ]
            )
        }
    }
