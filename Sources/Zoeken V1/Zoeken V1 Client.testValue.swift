    ...
    func search(
      ...
      if let handelsnaam = handelsnaam {
        filteredResults = filteredResults.filter {
          $0.handelsnaam.localizedCaseInsensitiveContains(handelsnaam)
        }
      }

      if let straatnaam = straatnaam {
        filteredResults = filteredResults.filter {
          $0.straatnaam?.localizedCaseInsensitiveContains(straatnaam) ?? false
        }
      }

      if let plaats = plaats {
        filteredResults = filteredResults.filter {
          $0.plaats?.localizedCaseInsensitiveContains(plaats) ?? false
        }
      }
