...
extension Naamgeving {
  public enum Vestiging: Codable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
      ...
      if let eersteHandelsnaam = try container.decodeIfPresent(
        String.self,
        forKey: .eersteHandelsnaam
      ),
        let handelsnamen = try container.decodeIfPresent([Handelsnaam].self, forKey: .handelsnamen)
      {
        ...
        throw DecodingError.dataCorruptedError(
          ...
          in: container,
          debugDescription:
            "Unable to decode either CommercieleVestiging or NietCommercieleVestiging"
        )
      }
    }
