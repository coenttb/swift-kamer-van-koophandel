...
struct BasisprofielClientTests {
  ...
  func testGetEigenaar() async throws {
    @Dependency(AuthenticatedClient.self) var client
    #expect(
      try await client.getEigenaar(validKvKNumber, nil).rsin == Basisprofiel.Eigenaar.kvk.rsin
    )
  }
  ...
  func testGetHoofdvestiging() async throws {
    ...
    #expect(
      try await client.getHoofdvestiging(validKvKNumber, nil).vestigingsnummer
        == Basisprofiel.Vestiging.kvkHoofdVestiging.vestigingsnummer
    )
  }
